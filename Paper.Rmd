...
p <- ggplot(ridership_trend_long, aes(x=Year, y=TotalRidership, color=Type, group=Type)) +
  ...
  geom_line(data=ridership_data, aes(x=Year, y=PredictedBusROC_poly, group=2), color="hotpink", linetype="dotted", size=2) +
  # Add the regression lines for predicted rail data (explicit aesthetics)
  geom_line(data=ridership_data, aes(x=Year, y=PredictedRailROC_poly, group=2), color="darkorange", linetype="dotted", size=2) +
  scale_y_continuous(labels = scales::label_comma(), breaks = scales::pretty_breaks(n = 16)) +
...
p + annotate("text", x=last_year, y=ridership_data$PredictedBusROC[ridership_data$Year == last_year], label="Predicted Bus Linear", color="blue", hjust=1.1, vjust=6.8) +
  annotate("text", x=last_year, y=ridership_data$PredictedRailROC[ridership_data$Year == last_year], label="Predicted Rail Linear", color="red", hjust=1.1, vjust=6) +
  annotate("text", x=last_year, y=ridership_data$PredictedRailROC_poly[ridership_data$Year == last_year], label="Predicted Bus Quadratic", color="hotpink", hjust=1.1, vjust=8) +
  annotate("text", x=last_year, y=ridership_data$PredictedRailROC_poly[ridership_data$Year == last_year], label="Predicted Rail Quadratic", color="darkorange", hjust=1.1, vjust=10)
```
