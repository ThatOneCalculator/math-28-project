...
combined_data_long <- combined_data_long %>%
  filter(!(Type == "Total.Monthly.Bus_poly" & TotalRidership < -10000000 & Prediction == "Predicted"))
...
# Add predicted lines for past years (where we have actual data)
predicted_data_long <- all_years %>%
  select(Year, Total.Monthly.Bus, Total.Monthly.Rail, Total.Monthly.Bus_poly, Total.Monthly.Rail_poly) %>%
  pivot_longer(cols = c("Total.Monthly.Bus", "Total.Monthly.Rail", "Total.Monthly.Bus_poly", "Total.Monthly.Rail_poly"),
               names_to = "Type",
               values_to = "TotalRidership") %>%
  mutate(Prediction = "Predicted") %>%
  filter(Year <= max(ridership_data$Year))

# Overlay this on the plot with a different line type
p <- p +
  geom_line(data = predicted_data_long, aes(x = Year, y = TotalRidership, color = Type, linetype = Prediction))
